// internal/entity/eye_controller.go
package entity
...
// EyeController manages eye animations for characters
...
// NewEyeController creates a new eye controller
...
// SetSpriteSheet sets the eye sprite sheet
...
// Update handles eye animation updates
...
// updateBlinking manages the blinking state and animation
func (c *EyeController) updateBlinking(dt time.Duration) {
	if c.isBlinking {
		...
		if c.animation.IsFinished() {
			c.isBlinking = false
			c.blinkTimer = 0
			c.blinkInterval = 180 + rand.Intn(120) // 3-5 seconds
		}
		return
	}
...
// This maintains the exact same behavior as the original code
...
func (c *EyeController) GetPosition(currentAnim string, currentFrame int) common.Vector2 {
...
func (c *EyeController) SetFlipX(flip bool) {
	c.flipX = flip
}
...
// Methods to satisfy the AnimationController interface if needed
func (c *EyeController) GetCurrentAnimation() string {
	switch c.direction {
	case LookingCenter:
		return "center"
	case LookingRight:
...
func (c *EyeController) PlayAnimation(name string) {
	// No-op for interface compatibility
}

func (c *EyeController) ReverseAnimation() {
	// Flip the eyes horizontally
	c.flipX = !c.flipX
	c.updateSprite()
}

func (c *EyeController) IsReversed() bool {
	return c.flipX
}
